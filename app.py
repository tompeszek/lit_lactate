...
def main():
    ...
    # Automatically analyze data if there are enough valid rows
    if len(st.session_state.step_data) >= 3:
        ...
        if results is not None:
            ...
            # Format columns: intensity as whole numbers, lactate to 1 decimal
            format_dict = {}
            ...
            st.info("Enter valid data to see analysis results (at least 3 rows with positive values)")
    else:
        st.info("Add at least 3 rows of data to see analysis results")
        ...
        if len(erg_tests) >= 1:
            ...
            for zone in zones:
                ...
                zone_data.append({
                    ...
                    'Power (W)': f"{pr['median_bottom']}-{pr['median_top']}",
                    'Pace (/500m)': f"{pace_top_str}-{pace_bottom_str}",
                    'HR (bpm)': f"{hr['median_bottom']}-{hr['median_top']}" if hr else 'N/A',
                    'Lactate (mmol/L)': f"{lac['median_bottom']}-{lac['median_top']}" if lac else 'N/A'
                })
